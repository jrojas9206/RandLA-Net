...
def prepare_several_sets(args):
    experiment_folders = [a_obj for a_obj in os.listdir(args.inputDir) if(os.path.isdir(os.path.join(args.inputDir, a_obj)))]
    for a_experiment in experiment_folders:
        data_folder = os.path.join(args.inputDir, a_experiment, args.whrootbase)
        for dataset_type in [a_set for a_set in args.whfolders.split(',')]:
            print(" -> Working on: %s" %(os.path.join(a_experiment, args.whrootbase, dataset_type)))
            prepare_data_generic(data_folder, 
                                args.outputDir, 
                                grid_size=args.gridSize, 
                                verbose=args.verbose, 
                                protocol=args.ExpProtocol, 
                                dataset=dataset_type, 
                                annCol=args.annColumn, 
                                colorCol=[int(a_feature) for a_feature in args.featureCols.split(',')]
                                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wholedataset", help="Process a group of dataset with the defined structure on the wh options", action="store_true")
    parser.add_argument("--whfolders", type=str, help="main folders to process", default="training,test")
    parser.add_argument("--whrootbase", type=str,help="Define the base folder that contain the whfolders, default: root", default="root")
    args = parser.parse_args()
    print("-> Prepare data to randlanet model")
    feature_col = args.featureCols.split(",")
    feature_col = [ int(i) for i in feature_col ]
    if(not args.wholedataset):
        prepare_data_generic(args.inputDir, args.outputDir, grid_size=args.gridSize, verbose=args.verbose, 
                            protocol=args.ExpProtocol, dataset=args.datasetType, annCol=args.annColumn, 
                            colorCol=feature_col)
    else:
        prepare_several_sets(args)