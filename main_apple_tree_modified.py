...
class AppleTree:
    ...
    def load_sub_sampled_clouds(self, sub_grid_size):
        ...
        for i, file_path in enumerate(files):
            ...
            print("    -> pc size: %s" %(str(data.shape)))
...
def train_synthetic_HiHiRes(inputDir, outputDir, parameters=None):
    global cfg
    cfg = cfg_synthetic
    #print("Defined number of points: %i" %(cfg.num_points))
    path2save_log = os.path.join(outputDir, "train_log")
    if(not os.path.isdir(outputDir)):
        os.mkdir(outputDir)
    if(not os.path.isdir(path2save_log)):
